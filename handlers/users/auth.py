...
async def back_choose_lang(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(_("Выбери язык:"), reply_markup=languages.keyboard)
...
async def choose_lang(callback: types.CallbackQuery, state: FSMContext, user: User):
    if callback.data in config.LANGUAGES.keys():
        ...
        try:
            await bot.edit_message_text(_("Привет!", locale = callback.data), chat_id=user.tele_id, message_id=user.welcome_message_id)
        except MessageNotModified:
            pass
        await callback.answer(_("Язык установлен", locale = callback.data))
        await callback.message.edit_text(_("Выберите ваш факультет:", locale = callback.data), reply_markup=keyboard)
...
async def auth_complete(callback: types.CallbackQuery, state: FSMContext, user: User):
    ...
    await state.finish()
    await callback.message.edit_text(_("Вы успешно прошли регистрацию!"))
    await MenuStates.mediate.set()
    keyboard = await menu.get_keyboard(user)
    await callback.message.delete()
    msg = await callback.message.answer(_("Вы успешно прошли регистрацию!"), reply_markup=keyboard)
