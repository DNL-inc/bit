...
def group_markup(faculty_id, course, message):
    ...
    groups = session.query(Group).filter(Group.faculty == faculty_id, Group.course == course).all()
    for group in groups:
        if user.group == group.id:
            markup.add(types.InlineKeyboardButton(text=group.title+' ✔', callback_data="group-"+str(group.id)))
        else: markup.add(types.InlineKeyboardButton(text=group.title, callback_data="group-"+str(group.id)))
    markup.add(types.InlineKeyboardButton(text='Назад', callback_data="backChooseGroup"))
    return markup
...
class GroupPanel:
    ...
    def choose_course(self, message):
        ...
        self.bot.edit_message_text(chat_id=message.from_user.id, message_id=message.message.message_id, text='Виберіть курс:', reply_markup=course_markup())
    ...
    def call_save_group(self, message):
        ...
        self.bot.edit_message_text(chat_id=message.from_user.id, message_id=message.message.message_id, text="Виберіть группу:", reply_markup=group_markup(self.faculty, self.course, message))