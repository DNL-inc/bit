...
def start(message):
    ...
    bot.send_message(message.from_user.id, """Как пользоваться

Чтобы подписаться на расписание группы нажми «Группа», выбери свой факультет, курс и группу, затем переходи в «Расписание» и выбери нужный день, чтобы увидеть расписание. 
